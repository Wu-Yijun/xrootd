 ...
 * - **内部处理**：从底层的 AnyObject 中提取出 std::vector<XrdCl::XAttrStatus> 指针，通过 TSFN 跳回 V8 主线程后，遍历向量中的每个元素，提取 name、状态码及错误信息，组装为 JS 数组，随后释放状态向量、释放 TSFN 并 delete this 自销毁。
 * - **返回结果**：向 JS Promise 成功 Resolve 包含 { name, isOk, code, message } 的 JS Array，失败 Reject `Napi::Error`。
 */
 ...
 * 
 * - **接受内容**：Napi::Env 环境句柄、Napi::Promise::Deferred Promise 延迟对象、操作名称 opName。
 * - **内部处理**：从底层的 AnyObject 中提取出 std::vector<XrdCl::XAttr> 指针，通过 TSFN 跳回 V8 主线程后，调用 Utils::XAttrVectorToObject 将其反序列化为 Record<string, string> 结构的 JS Object，随后释放属性向量、释放 TSFN 并 delete this 自销毁。
 * - **返回结果**：向 JS Promise 成功 Resolve 包含属性键值对的 JS Object，失败 Reject `Napi::Error`。
 */
