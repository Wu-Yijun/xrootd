 ...
 * @details 适用于 XrdCl::File 实例发起的无数据载荷返回的控制操作（如 Open, Close, Sync, Truncate
 * 等）。
 *
 * - **接受内容**：Napi::Env 环境句柄、Napi::Promise::Deferred Promise 延迟对象、操作名称 opName。
 * - **内部处理**：通过 TSFN 将后台网络线程跳回 V8 主线程；操作成功则忽略底层
 * AnyObject，失败则转换错误码。完成后释放 TSFN 并 delete this 自销毁。
