...
public:
  FileWriteHandler(Napi::Env env, Napi::Promise::Deferred deferred,
                   Napi::Buffer<char> jsBuffer)
      : deferred_(deferred) {
  ...
  virtual void HandleResponse(XrdCl::XRootDStatus *status,
    ...
    tsfn_.BlockingCall(
        [this, statusStr = status->ToString(),
         isOk = status->IsOK()](Napi::Env env, Napi::Function /*jsCallback*/) {
          if (isOk) {
