 ...
 * @details 适用于不需要返回具体数据载荷的操作（如 Rm, MkDir, RmDir, ChMod, Mv, Truncate, Ping 等）。
 * 
 * - **接受内容**：Napi::Env 环境句柄、Napi::Promise::Deferred Promise 延迟对象、操作名称 opName。
 * - **内部处理**：通过 Napi::ThreadSafeFunction 将后台网络线程跳回 V8 主线程；如果操作成功则忽略底层 AnyObject 数据，失败则转换错误码。完成后释放 TSFN 并 delete this 自销毁。
