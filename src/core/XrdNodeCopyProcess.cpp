...
public:
    CopyRunWorker(Napi::Env env, Napi::Promise::Deferred deferred, 
                  XrdNodeCopyProcess* owner, 
                  XrdCl::CopyProcess* cp, 
                  std::vector<XrdCl::PropertyList*>& results, 
                  std::atomic<bool>& cancelFlag, 
                  const Napi::FunctionReference& progressCb)
        : Napi::AsyncWorker(env, "CopyRunWorker"), deferred_(deferred), owner_(owner), cp_(cp), results_(results) {
    ...
    void OnOK() override {
        ...
        for (size_t i = 0; i < results_.size(); i++) {
            ...
            uint64_t size = 0;
            if (p->Get("size", size)) resObj.Set("size", Napi::Number::New(env, static_cast<double>(size)));
            
            std::string srcCksm;
            if (p->Get("sourceCheckSum", srcCksm)) resObj.Set("sourceCheckSum", Napi::String::New(env, srcCksm));
            
            std::string tgtCksm;
            if (p->Get("targetCheckSum", tgtCksm)) resObj.Set("targetCheckSum", Napi::String::New(env, tgtCksm));

            std::string realTarget;
            if (p->Get("realTarget", realTarget)) resObj.Set("realTarget", Napi::String::New(env, realTarget));
...
public:
    CopyPrepareWorker(Napi::Env env, Napi::Promise::Deferred deferred, XrdNodeCopyProcess* owner, XrdCl::CopyProcess* cp)
        : Napi::AsyncWorker(env, "CopyPrepareWorker"), deferred_(deferred), owner_(owner), cp_(cp) {
        owner_->Ref();
    }

    ~CopyPrepareWorker() override {
        owner_->Unref();
    }
...
Napi::Value XrdNodeCopyProcess::AddJob(const Napi::CallbackInfo& info) {
  ...
  if (config.Has("makeDir")) props.Set("makeDir", config.Get("makeDir").As<Napi::Boolean>().Value());
  if (config.Has("chunkSize")) props.Set("chunkSize", config.Get("chunkSize").As<Napi::Number>().Uint32Value());
  if (config.Has("parallelChunks")) props.Set("parallelChunks", static_cast<uint8_t>(config.Get("parallelChunks").As<Napi::Number>().Uint32Value()));
...
Napi::Value XrdNodeCopyProcess::Run(const Napi::CallbackInfo& info) {
  ...
  auto* worker = new CopyRunWorker(env, deferred, this, cp_, jobResults_, isCancelled_, progressCallback_);
  worker->Queue();
...
Napi::Value XrdNodeCopyProcess::SetEventListener(const Napi::CallbackInfo& info) {
  ...
  if (info.Length() < 2 || !info[0].IsString() || !info[1].IsFunction()) {
    Napi::TypeError::New(env, "Expected (eventName: string, listener: function)").ThrowAsJavaScriptException();
    return env.Null();
  }
