...
class XrdNodeFile : public Napi::ObjectWrap<XrdNodeFile> {
 ...
 private:
  ...
  /**
   * @brief 从本地 fd 异步写入数据到文件。
   *
   * JS 签名: `writeFd(offset: bigint, size: number, fd: number, fdoff?: bigint): Promise<void>`
   *   - info[0] (bigint): 写入的起始字节偏移量。
   *   - info[1] (number): 写入大小。
   ...
   * @return Promise<void>
   */
  Napi::Value WriteFd(const Napi::CallbackInfo& info);
   ...
   * @brief 异步分页式写入数据并校验。
   *
   * JS 签名: `pgWrite(offset: bigint, size: number, buffer: Buffer, cksums: number[]):
   * Promise<void>`
   * @param info
   ...
   * @param info
   *   - info[0] (Array): 包含要克隆的数据范围详情数组 (每个项包括 srcFile, dstOffset, srcOffset,
   * length)。
   * @return Promise<void>
   */
